...
## Wat is dit
SportCoach AI — een Nederlandstalige AI-sportcoach-webapp (Next.js, PWA) voor één atleet (Taco). Integreert Garmin-data, dagelijkse check-ins, AI-coach chat (Claude), weekrapporten, MyFitnessPal-voeding, materiaal-tracking en multisport.
...
- **Pure logica** in `src/lib/*` (bijv. `equipment.ts`, `swim.ts`, `training-load.ts`, `periodization.ts`), UI in `src/components/*`.
